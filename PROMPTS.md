...
업데이트 안내 (클로드 아티팩트의 외부 API 정책이 변경으로 아래의 두 프롬프트를 다음과 같이 대체해 실행해 주세요.)
```diff
- 오늘의 날씨와 주요 뉴스를 보여 주는 나만의 시작 홈페이지를 만들고 싶어 (X)
```
```diff
- 밝고 깔끔한 스타일을 선호해. 시계와 검색바도 추가해 줘. 기술 관련 뉴스를 보여 줘 (X)
```
...
이후 추가 질문에 답하면서 첫 아티팩트를 완성해 보세요. 나머지 "게시", "커스터마이즈" 관련 내용은 책의 내용 그대로 실행하시면 됩니다.
...
```
[현재 시간], [오늘의 할 일] 아이템을 한 열에 두 개씩 배치하고, 메모지 안에 각각 들어 있는 디자인으로 수정해 줘.
```
...
업데이트 안내
```diff
- npm install -g @anthropic-ai/claude-code (X)
```
(설치 방법이 변경되었습니다. 아래 **[업데이트된 설치 가이드]** 를 참고해 주세요.)
